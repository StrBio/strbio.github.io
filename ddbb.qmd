...
while structure superposition displays the structures using a given criteria, usually from a previous structural  alignment. Thus, based on that criteria, superposition tries to minimize the distance between structures by finding a transformation that produces either the lowest root-mean-square deviation
...
[*cealign*]{.underline} algorithm implemented in Pymol. The RMSD and the number
of aligned residues for each pair of structures is
...
Although differences between primary and secondary databases are blurry nowadays because most databases currently contain data from different sources, we still can make some differences.
The main protein primary databases are NCBI Protein for protein sequences and
...
Link [here](https://www.evernote.com/shard/s62/nl/6844171/0e51e43f-5256-4d27-fd1e-7b7cc48db50a?title=StrBio%202023/2024%20-%20(B)%20Protein%20DDBB)
:::
