...
On Windows, Linux, or macOS, there are a number of options to use
[PyMol](https://www.pymol.org/) on your computer without a paid license:
...
2.  Open Access version from GitHub
    (<https://github.com/schrodinger/pymol-open-source>), also available from
    ...
    -   NOTE: for new macOS you may need to install some dependencies:

        ``` bash
        brew install tcl-tk
        ```

3.  Open Access version from
    [`Conda`](https://anaconda.org/channels/conda-forge/packages/pymol-open-source/overview):

    ``` bash
    conda install -c conda-forge pymol-open-source
    ```

4.  Open Access version from
    [`PIP`](https://pypi.org/project/pymol-open-source/):
...
This exercise is designed as a self‑guided tutorial to help you learn how to
work with protein structures in PyMOL and create publication‑quality figures.
The main goal is to strengthen your ability to work independently with
structures. In many sections, you’ll find several different ways to achieve the
same or similar results. Please avoid copying and pasting without thinking and
**take the time to understand each step and learn from it**.
:::
...
-   **Save your session when you accomplish a wanted display:**
    ...
    -   The commands `get_view` and `set_view` are very useful to reproduce a
        particular view of a protein, particularly when making a movie. See
        <https://pymolwiki.org/Set_view>\
:::
...
*⚠️️ Note that the options 1 and 2 below are alternative ways to get the same
results. Try only one option each time starting over from 6KI3 structure.⚠️❗️*
...
If your protein is a multimer, you can download a biological assembly or
biological unit, use the assembly setting or use type=pdb1 (monomer), type=pdb2
(dimer) and so on. Once you have learned to create and remove objects, try
`fetch 6KI3, type=pdb1` to fetch and display only a monomer of `6KI3`
:::
...
From version 1.7.2, there is also a command to align multiple structures with a
reference molecule, using
...
Then, clicking two atoms to create a dashed line showing the distance in
Ångströms.
...
## 9. How to save a high-res picture: RAY IT!

::: callout-warning
## Ray only for high-resolution images
...
![](images/f51cf939-2d21-4220-a0dd-33a038f4f7af.png){width="600"}
...
::: callout-tip
Please let me know if any of the links below are broken or if you would like to
add new links to other tricks.
:::
...
# PART B: Paper-style protein figure challenge (optional)
...
substrate/cofactor molecule can be a good challenge. You may use PyMOL,
ChimeraX, Mol\* or any other application, including Py3Dmol in a Jupyter
Notebook.
